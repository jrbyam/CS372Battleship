#pragma once

#include "il2cpp-config.h"

#ifndef _MSC_VER
# include <alloca.h>
#else
# include <malloc.h>
#endif

#include <stdint.h>

// System.Diagnostics.TraceListenerCollection
struct TraceListenerCollection_t4109843382;

#include "mscorlib_System_Object837106420.h"
#include "System_System_Diagnostics_SourceLevels131882185.h"

#ifdef __clang__
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Winvalid-offsetof"
#pragma clang diagnostic ignored "-Wunused-variable"
#endif

// System.Diagnostics.TraceSourceInfo
struct  TraceSourceInfo_t412274093  : public Il2CppObject
{
public:
	// System.Diagnostics.SourceLevels System.Diagnostics.TraceSourceInfo::levels
	int32_t ___levels_0;
	// System.Diagnostics.TraceListenerCollection System.Diagnostics.TraceSourceInfo::listeners
	TraceListenerCollection_t4109843382 * ___listeners_1;

public:
	inline static int32_t get_offset_of_levels_0() { return static_cast<int32_t>(offsetof(TraceSourceInfo_t412274093, ___levels_0)); }
	inline int32_t get_levels_0() const { return ___levels_0; }
	inline int32_t* get_address_of_levels_0() { return &___levels_0; }
	inline void set_levels_0(int32_t value)
	{
		___levels_0 = value;
	}

	inline static int32_t get_offset_of_listeners_1() { return static_cast<int32_t>(offsetof(TraceSourceInfo_t412274093, ___listeners_1)); }
	inline TraceListenerCollection_t4109843382 * get_listeners_1() const { return ___listeners_1; }
	inline TraceListenerCollection_t4109843382 ** get_address_of_listeners_1() { return &___listeners_1; }
	inline void set_listeners_1(TraceListenerCollection_t4109843382 * value)
	{
		___listeners_1 = value;
		Il2CppCodeGenWriteBarrier(&___listeners_1, value);
	}
};

#ifdef __clang__
#pragma clang diagnostic pop
#endif
